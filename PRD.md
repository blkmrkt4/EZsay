...
**Style-guide nudges (added 2026-07-15).** Users won't find the Style Rules tab on their own, and the scan can only check what an institution requires if the rules exist. Two non-blocking nudges appear while the user has no style rules at all (`/api/style-preferences` GET now returns `styleGuideParsedAt` — latest across all the user's rows — and `hasAnyPreferences`; the client's `hasStyleRules` refetches on nav changes so uploading a guide clears the nudges immediately):
1. **Intake:** an amber "Do you have a style guide?" card under every question, with an "Open Style Rules" button. While style rules are missing, the "Skip and start scanning" escape hatch is replaced by an amber "Highly recommended: go through the style guide first" link (a rules-less scan misses what the user is looking for); both style-guide exits silently PATCH the answers given so far, so intake progress survives — the user finishes the rest via "Edit context". With style rules present, the muted "Skip and start scanning" link returns.

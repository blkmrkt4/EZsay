...
| 27 | Turnitin/detector parity | Closed, not deferred: Turnitin has no public API — it's licensed only to institutions via LMS integration (Canvas/Blackboard/Moodle), so there is no "real Turnitin" for EzSay to call or calibrate against at any budget. Third-party proxy APIs (GPTZero, Originality.ai, Copyleaks) were considered and explicitly declined — per-scan cost, ToS/legal exposure reselling another vendor's detection inside a paid product, and still no verified parity with the detector students actually care about. The honest-estimate framing already shipped (decision #23) is the permanent answer, not a placeholder. See CLAUDE.md "Not in scope for V1" — detector score simulation stays out of scope by design, not by omission | 2026-08-05 |
...
## 26. Real-essay regression suite for the corruption bug class (2026-08-05)

**Why:** every layer of the edit/export pipeline had been tested in isolation (or, for `flags/resolve` and the bulk-fix routes, not at all) — nothing tested the full chain of a realistic multi-step edit session followed by a docx-surgery export, and nothing had been run against real-world essay structures (tables, footnotes, embedded objects) rather than only the synthetic fixture in `docx-surgery.test.ts`.

**Extraction (behavior-preserving):** the corruption-prevention decision logic in `app/api/flags/resolve/route.ts` — `locateSpan` (span re-anchor by search) and the whole-section-vs-artifact replacement decision, including the truncation guard and blocking corruption check — moved into `lib/analysis/flag-resolution.ts` as `locateSpan` + `computeFlagReplacement`, a pure exported module with identical logic, error messages, and status-code mapping. The route now calls it. Verified behavior-preserving via diff review, `tsc`/`build`, and a live accept + idempotency + sibling-auto-skip check against the designated test document (reset afterward). This is the ONLY reason to ever touch that route's decision logic again — the pure module is what future edits should modify, with the route staying a thin wrapper.

**Suite:** `__tests__/essay-pipeline-regression.test.ts` + `__tests__/helpers/no-duplication.ts`. Reads real `.docx` essays from `ESSAY_FIXTURES_DIR` (default `/Users/blkmrkt/Documents/Calum Essays`) — **never committed** (`.gitignore`: `__tests__/fixtures/essays/`), gracefully **skipped** (not failed) on any machine without that folder. Per essay: ingestion sanity, a zero-edit round trip through `exportPreservedDocx` (must preserve exactly or refuse with a named reason — e.g. one real essay has an embedded OLE object and correctly falls back to `unsupported-construct`), a simulated realistic edit session using the REAL `computeFlagReplacement`/`locateSpan` (whole-section rewrites, two sequential edits landing in the same section — the exact historical bug shape, an artifact re-anchor-by-search, a stale-artifact refusal), and a multi-paragraph rewrite case. Assertions re-ingest the exported file via the same mammoth+`parseAndSplit` pipeline real uploads use and compare paragraph-for-paragraph (normalized the same way re-ingestion normalizes, so a benign double-space from a narrow word removal isn't mistaken for corruption), plus a zip-entry byte-identity check on every part outside `word/document.xml`.

**Confirmed the suite has teeth:** temporarily reverted the `computeFlagReplacement` fix (forced every patternType through the narrow-splice branch) — 7 of 17 tests failed immediately across multiple essays with the duplicated text visible in the assertion diff. Reverted; suite passes clean at 17/17 (80/80 including all prior suites).

Run: `npm test` (auto-skips without the fixtures folder). To point at a different local copy: `ESSAY_FIXTURES_DIR=/path/to/essays npm test`.

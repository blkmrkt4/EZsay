...
**Style-guide nudges (added 2026-07-15).** Users won't find the Style Rules tab on their own, and the scan can only check what an institution requires if the rules exist. Two non-blocking nudges appear until the user has ENGAGED with their style guide — engagement = an uploaded/parsed guide (`styleGuideParsedAt`) OR any Style Rules save after the nudge feature shipped (`styleRulesTouchedAt` = max `updatedAt` across the user's `userStylePreferences` rows, compared to a hardcoded `STYLE_RULES_ENGAGEMENT_EPOCH` of 2026-07-15 — preferences from the old pre-nudge wizard deliberately do NOT count; the user must go in and do something). Artifact-override tweaks live in another table and don't clear the nudge — only guide upload and style-preference saves do. The client's `hasStyleRules` refetches on nav changes so acting on the nudge clears it immediately:

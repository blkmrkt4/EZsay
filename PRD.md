...
- **No new infrastructure.** The browser tab drives generation: `runGenerationLoop` in `app/w/page.tsx` walks the open flags in document order and calls `/api/suggest-all` in **small slices** (flag 1 first and alone so the user can start immediately, then slices of 2). Each request is short enough to survive serverless limits; `maxDuration = 120` is set on `/api/suggest-all`, `/api/suggest`, and `/api/scan`. Generation runs while the UI is fully interactive.

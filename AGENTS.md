...
- **Below that — numbered replacement options.** Each option shows:
  ...
  - **Backend:** `app/api/flags/resolve` splices a `manualText` replacement into the flagged span (previously only the `optionId` path updated `sections.currentText`; manual/"Edit myself" edits were stored on `flags.manualReplacement` but never applied — this fixed that, so copy/paste remixing via "Edit myself" actually changes the document).

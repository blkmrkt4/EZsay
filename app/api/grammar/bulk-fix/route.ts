...
export async function POST(request: NextRequest) {
  ...
  const allFindings = (doc.grammarResults as GrammarFinding[]) || [];
  ...
  // Group by section
  const bySection = new Map<string, GrammarFinding[]>();
  ...
  for (const [sectionId, sectionFindings] of bySection) {
    ...
    for (const finding of sorted) {
      ...
      if (actual === finding.originalText) {
        text =
  ...
  const newScore = Math.max(0, 100 - remaining.length * 3);

  await db
    .update(documents)
    .set({
      grammarResults: remaining,
      grammarScore: newScore,
      updatedAt: new Date(),
    })

...
export default function AnalysisPanel({ document: doc, versions, flags, plagiarismResults, plagiarismLoading, scanning, documentText, suggestionsGenerating, suggestionsProgress, toneCheckDone, plagiarismEnabled, toneEnabled, onStartEditing }: AnalysisPanelProps) {
...
function ScoreSpectrum({ label, score, interpretation, lowLabel, highLabel, lowerIsBetter, loading, expanded, onClick }: {
  ...
  // Gradient always goes red → yellow → green (left to right)
  // For "lower is better" scores, the marker position is inverted (low score = right/green side)
  const markerPosition = hasScore
    ? lowerIsBetter ? 100 - score : score
    : 0;

  return (
    <button
      onClick={onClick}
      className={`w-full text-left rounded-lg border p-3 transition-colors ${expanded ? "border-blue-400 bg-blue-50/30 ring-1 ring-blue-400" : "border-gray-200 hover:border-gray-300 hover:bg-gray-50"}`}
    >
      ...
      <div className="relative h-3 rounded-full overflow-hidden" style={{
        background: hasScore ? "linear-gradient(to right, #ef4444, #eab308 50%, #22c55e)" : "#e5e7eb",
      }}>
        {hasScore && (
          <div className="absolute top-[-1px] h-[calc(100%+2px)] w-1.5 bg-gray-900 rounded-full shadow" style={{ left: `${markerPosition}%`, transform: "translateX(-50%)" }} />
        )}
      </div>

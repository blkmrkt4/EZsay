...
- **Below that — numbered replacement options.** Each option shows:
  ...
  - **Comparison stage (`MorphStage`), hybrid per-box choosing:** a "{N} edits to make here" box shows the passage once, and at **every** divergence stacks all option variants inline, numbered to match the Choices panel. Two ways to choose: (a) **whole-option** — click Option N in the Choices panel on the right; one click, applies option N everywhere, advances (logged as `option_selected`); or (b) **per-box mix** — click a numbered variant inside any box in the centre to override just that spot. Building a mix reveals an "Apply my mix & continue" button; on commit, if every box landed on the same option it's submitted as that option (correct logging), otherwise the stitched text is submitted as a manual replacement. `boxChoices` / `activeBoxIdx` / `hasCustomMix` state in `app/w/page.tsx`. Stage only renders for 2+ options that differ; artifacts / single option / no differences fall back to the "Suggested replacements" cards.
  - **Step-through + source highlight:** the active box (the one being decided) gets a violet ring; picking a variant advances to the next box. The Document panel highlights the **whole flagged passage faint violet** and the **slice the active box replaces in strong violet** (`computeActiveSourceSpan` finds that slice by fuzzily matching the shared text on either side of the box in the original — falls back to the whole span). Violet = "what's being edited"; amber = the option variants. (History: a "morph/preview-on-select" stage, a bright flash, and a "▶ Tour changes" walkthrough were built then removed on 2026-06-08 as confusing; per-box mixing was added 2026-06-08 at the user's request as the deliberate, structured version of that control.)
  - **Backend:** `app/api/flags/resolve` now splices a `manualText` replacement into the flagged span too (previously only the `optionId` path updated `sections.currentText`; manual/"Edit myself" edits were stored on `flags.manualReplacement` but never applied — this fixed that and enables the custom-mix commit).

...
- **Below that — numbered replacement options.** Each option shows:
  ...
  - **Comparison stage (`MorphStage`):** a "Compare the options in context" box shows the passage once, and at **every** divergence stacks all option variants inline, numbered to match the Choices panel. It is **read-only** — purely for comparing in context. The choice is committed in the Choices panel on the right, where clicking an option applies it and advances to the next edit (one action, not a preview). The stage only renders when there are 2+ options that actually differ; for artifacts / a single option / no differences, the fallback "Suggested replacements" cards render instead.
  - **Clear split:** compare in the middle, choose on the right. There is no per-spot mix-and-match — the options are three *complete* rewrites of one passage; picking option N applies the whole option-N rewrite. (A "morph/preview-on-select" stage, a bright flash, and a "▶ Tour changes" walkthrough were built and then removed on 2026-06-08: the dual numbered controls (read-only center vs. commit-on-click right) confused the model of "how do I actually choose," and the tour added little. Don't reintroduce a second interactive option control in the center.)
  - A one-line note in muted text explaining what changed
  - (Rationale, 2026-06-05→08: full-paragraph rewrites of large spans made adjacent options near-identical; option-vs-option alignment + all-variants-inline-in-context make the real difference visible without re-reading the whole paragraph, while keeping a single, unambiguous place to choose.)

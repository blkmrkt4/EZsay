...
- **Below that — numbered replacement options.** Each option shows:
  ...
  - **Comparison stage (`MorphStage`, Directions A+C):** a "Compare in context" box shows the passage once, and at **every** divergence stacks all option variants inline, numbered, so they can be compared in context without clicking back and forth. The selected option's row is highlighted throughout; clicking any row selects that option. Switching options re-mounts the chosen rows so they **flash bright** (`ez-flash` keyframe in `globals.css`) — the swap is unmistakable, not a quiet fade. The header reads "Option N selected — click <other numbers> at any highlight to compare".
  - **Change walkthrough (Direction C):** when there is **more than one** divergence, a "▶ Tour changes" control steps (auto every 1.5s, or `‹`/`›`) through them, ringing the active stop (`ez-ring`, a persistent pulsing ring) and scrolling it + the Document panel into view. Hidden when options differ in a single place. State: `activeChangeIdx` / `isPlayingChanges` / `changeRefs` in `app/w/page.tsx`.
  - A one-line note in muted text explaining what changed
  - (Rationale, 2026-06-05/06: full-paragraph rewrites of large spans made adjacent options near-identical; option-vs-option alignment, all-variants-inline-in-context, a bright flash on switch, and a guided walkthrough make the real choice visible without re-reading the whole paragraph.)

...
- **Below that — numbered replacement options.** Each option shows:
  ...
  - **Morph stage (Direction A):** a "Preview — Option N in context" box shows the passage once; the shared skeleton stays put and only the diverging spans swap to the selected option, animating in place (`MorphStage` + the `ez-morph` keyframe in `globals.css`). Clicking an option card or choice selects it and morphs the stage.
  - **Change walkthrough (Direction C):** a "▶ Tour changes" control steps (auto every 1.5s, or `‹`/`›`) through the divergent blocks, pulsing each (`ez-pulse`) and scrolling it + the Document panel into view. State: `activeChangeIdx` / `isPlayingChanges` / `changeRefs` in `app/w/page.tsx`.
  - A one-line note in muted text explaining what changed
  - (Rationale, 2026-06-05: full-paragraph rewrites of large spans made adjacent options near-identical; option-vs-option alignment + in-context morph + a guided walkthrough make the real choice visible without re-reading the whole paragraph.)
